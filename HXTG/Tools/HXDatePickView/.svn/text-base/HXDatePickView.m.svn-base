...
@interface HXDatePickView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
-(void)setFrameWith:(BOOL)isHaveNavControler{
    ...
    toolViewX = 0;
    toolViewH = _pickeviewHeight+ZHToobarHeight;
    toolViewY =0;
    ...
    toolViewW = [UIScreen mainScreen].bounds.size.width;
    self.frame = CGRectMake(toolViewX, toolViewY*2, toolViewW, toolViewH);
...
-(void)setUpPickView{
    ...
    toolViewW = [UIScreen mainScreen].bounds.size.width;
    pickView.frame=CGRectMake(0, ZHToobarHeight, toolViewW, pickView.frame.size.height);
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    toolViewW = [UIScreen mainScreen].bounds.size.width;
    datePicker.frame=CGRectMake(0, ZHToobarHeight, toolViewW, datePicker.frame.size.height);
...
-(UIToolbar *)setToolbarStyle{
    ...
    [lefttem setTintColor:UIColorHigRedTheme];
    
    UIBarButtonItem *centerSpace=[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    
    UIBarButtonItem *right=[[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(doneClick)];
    [right setTintColor:UIColorHigRedTheme];
...
-(void)remove{
    ...
    [UIView animateWithDuration:0.3
                     animations:^{
                         self.frame = CGRectMake(toolViewX, toolViewY+self.frame.size.height, toolViewW, toolViewH);
                     }
